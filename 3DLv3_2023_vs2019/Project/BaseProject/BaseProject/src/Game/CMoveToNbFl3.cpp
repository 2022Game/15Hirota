...
// rotの後にモデルパスを選択できる
CMoveToNbFl3::CMoveToNbFl3(const CVector& scale, const CVector& rot
	, float waitTime)
	: CNumberFloor3(CVector::zero, scale, rot)
...
CMoveToNbFl3::~CMoveToNbFl3()
{
	CStageManager::RemoveTask(this);
}

// 移動ポイントを追加
void CMoveToNbFl3::AddMovePoint(const CVector& point, float time)
{
	MovePoint mp;
	mp.point = point;
	mp.time = time;
...
void CMoveToNbFl3::SetReturnRoute(bool isReturn)
{
	mIsReturn = isReturn;
}

// 状態を切り替える
void CMoveToNbFl3::ChangeMoveState(EMoveState state)
{
...
// 移動ルートを更新
void CMoveToNbFl3::UpdateMoveRoute()
{
	// 順ルートで移動中
	if (!mIsReturnMove)
	{
